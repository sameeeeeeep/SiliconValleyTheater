"""
SiliconValley Theater — TTS Sidecar

Supports two engines (selectable via --engine):
  - pocket  (default): Pocket TTS (100M) — native voice cloning, CPU-only, ~1x RT
  - kokoro:            Kokoro (82M) via mlx-audio — fast (RTF ~0.3), preset/cloned voices

Both engines check the phrase cache first for instant playback of common reactions.

Usage:
    python server.py [--port 7893] [--engine pocket|kokoro]
...
app = FastAPI(title="SiliconValley TTS Sidecar")
...
# Voice → reference WAV for Pocket TTS cloning
